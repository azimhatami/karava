...
function Stat({icon, title, value, color}) {
  return(
    <div className={`col-span-1 grid grid-rows-2 grid-cols-[6.4rem_1fr] bg-ink-card p-4 rounded-lg gap-x-4 ${colors[color]}`}>
      <div className={`row-span-2 flex items-center justify-center aspect-square rounded-full ${colors[color]}`}>
        {icon}
      </div>
      <h5 className='font-bold text-ink-muted text-lg self-center'>{title}</h5>
      <p className='font-bold text-ink-text text-xl'>
